...
while($row =$stm->fetch()){
    ...
    $pdf->Cell(0,10,'Hour de reclamation : '.$row['Hour_reclamation'],0,1);
...
// $id_gestion = $_GET["ref"]; 
// $stmt = $db->prepare("SELECT * FROM convrsations WHERE id_gestion=:id_gest ORDER BY date_conversation ASC LIMIT 1");
// $stmt->execute([":id_gest" => $id_gestion]);


// while ($conversation=$stmt->fetch()) {
//     $pdf->MultiCell(0,10,'description: '.$conversation['conver_message'] ,0,1);
//     } 

//     $id_gestion = $_GET["ref"]; 
//     $id_user = $_SESSION["id"];
//     $stmt2 = $db->prepare("SELECT * FROM convrsations WHERE id_gestion=:id_gest");
//     $stmt2->execute([":id_gest" => $id_gestion]);
    
//     while ($msg=$stmt2->fetch()) {
//         if ($msg["id_utilisateur"] != $id_user) { // Added a condition to check if the message is from another person
//             $pdf->MultiCell(0,10,'reponds: '.$msg['conver_message'],0,1);
//         } 
//     }
        ...
        // $id_gestion = $_GET["ref"]; 
        // $id_user = $_SESSION["id"];
        // $stmt = $db->prepare("SELECT id_gestion,id_utilisateur,DISTINCT conver_message FROM convrsations WHERE id_gestion=:id_gest");
        // $stmt->execute([":id_gest" => $id_gestion]);
        // // Loop through the conversations and add them to the PDF
        // while ($conversation = $stmt->fetch(PDO::FETCH_ASSOC)) {
        //     if ($conversation["id_utilisateur"] !== $id_user) {
        //         $pdf->MultiCell(0,10,'reponds : '.$conversation['conver_message'],0,1);
        //     }
        //   }
    }
    $pdf->Ln(10);
